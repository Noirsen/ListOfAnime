...
fun AnimeListScreen(genre: String, onBackPressed: () -> Unit) {
    ...
    // State untuk toggle antara List dan Grid
    ...
    Scaffold(
            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = { isListView = !isListView }) {
                        Icon(
                            // Gunakan icon yang tersedia
                            imageVector = if (isListView) Icons.Filled.KeyboardArrowLeft else Icons.Filled.KeyboardArrowRight,
        ...
        Box(
            ...
            if (isListView) {
                AnimeListView(animeInGenre)
            } else {
                AnimeGridView(animeInGenre)
            }
        }
    }
}
...
@Composable
fun AnimeListView(animeList: List<Anime>) {
    LazyColumn(
        contentPadding = PaddingValues(horizontal = 32.dp, vertical = 16.dp),
        verticalArrangement = Arrangement.spacedBy(16.dp)
    ) {
...
@Composable
fun AnimeGridView(animeList: List<Anime>) {
    LazyVerticalGrid(
